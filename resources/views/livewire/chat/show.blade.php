...
<x-chat-layout :title="$chat->title">
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4">
                @if(Auth::user()->hasRole('admin') || $chat->created_by === Auth::id())
                    <a 
                        href="{{ route('chat.manage', $chat->slug) }}"
        ...
        @forelse($messages as $message)
            <div class="flex {{ $message->user_id === Auth::id() ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-xs lg:max-w-md">
                    <div class="flex items-start space-x-2 {{ $message->user_id === Auth::id() ? 'flex-row-reverse space-x-reverse' : '' }}">
                        <!-- Avatar -->
                        ...
                        <!-- Message Bubble -->
                        <div class="relative">
                            <div class="{{ $message->user_id === Auth::id() ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-900' }} px-4 py-2 rounded-lg shadow-sm">
                                <div class="text-xs {{ $message->user_id === Auth::id() ? 'text-blue-100' : 'text-gray-500' }} mb-1">
                                    {{ $message->user->name }}
                            ...
                            <!-- Message Actions -->
                            @if($message->user_id === Auth::id() || Auth::user()->hasRole('admin'))
                                <div class="absolute -top-2 {{ $message->user_id === Auth::id() ? '-left-8' : '-right-8' }} opacity-0 hover:opacity-100 transition-opacity">
                                    <button 
